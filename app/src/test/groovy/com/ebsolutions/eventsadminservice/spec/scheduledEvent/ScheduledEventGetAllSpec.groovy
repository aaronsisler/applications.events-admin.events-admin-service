...
class ScheduledEventGetAllSpec extends Specification {
    ...
    def "Get all scheduled events: URL Event Schedule id exists: Scheduled events exist for event schedule"() {
        ...
        and: "the correct scheduled events are returned"
            ...
            Assertions.assertEquals(ScheduledEventTestConstants.GET_ALL_SCHEDULED_EVENTS_SCHEDULED_EVENT_THREE.getOrganizerIds().get(0), thirdScheduledEvent.getOrganizerIds().get(0))
            Assertions.assertEquals(ScheduledEventTestConstants.GET_ALL_SCHEDULED_EVENTS_SCHEDULED_EVENT_THREE.getOrganizerIds().get(1), thirdScheduledEvent.getOrganizerIds().get(1))
