...
class ScheduledEventSpec extends Specification {
    ...
    def "Get a scheduled event: URL Client id exists: Scheduled event exists"() {
    ...
    def "Create a scheduled event: URL Event Schedule id exists: Create Reoccurring Scheduled Event: Create fails given scheduled event's event schedule id is blank"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
    ...
    def "Create a scheduled event: URL Event Schedule id exists: Create Reoccurring Scheduled Event: Create fails given scheduled event interval is null"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
    ...
    def "Create a scheduled event: URL Event Schedule id exists: Create Reoccurring Scheduled Event: Create fails given scheduled event day is null and scheduled event interval is biweekly"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
        ...
        and: "the scheduled event's event schedule id does not match the URL event schedule id"
            ScheduledEvent newScheduledEvent = CopyObjectUtil.scheduledEvent(ScheduledEventTestConstants.CREATE_SCHEDULED_EVENT_REOCCURRING)
            newScheduledEvent.setScheduledEventDay(null)
            newScheduledEvent.setScheduledEventInterval(ScheduledEventInterval.MONTHLY)
    ...
    def "Create a scheduled event: URL Event Schedule id exists: Create Reoccurring Scheduled Event: Create fails given scheduled event day is null and scheduled event interval is monthly"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
    ...
    def "Create a scheduled event: URL Event Schedule id exists: Create Reoccurring Scheduled Event: Create scheduled event is successful"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
    ...
    def "Update a scheduled event: URL Event Schedule id exists: Update fails given scheduled event's event schedule id is blank"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
        ...
        and: "a scheduled event exists in the database"
            ...
            Assertions.assertEquals(HttpURLConnection.HTTP_OK, initResponse.code())
            Assertions.assertEquals(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getEventScheduleId(), initResponse.body().getEventScheduleId())

        and: "the scheduled event's event schedule id is blank"
            ScheduledEvent updatedEvent = CopyObjectUtil.scheduledEvent(initResponse.body())
            updatedEvent.setEventScheduleId("")
    ...
    def "Update a scheduled event: URL Event Schedule id exists: Update fails given scheduled event's event schedule id and URL event schedule id do not match"() {
        given: "the event schedule id is in the url"
            String scheduledEventsUrl = new StringBuffer()
                    .append(TestConstants.eventsAdminServiceUrl)
                    .append("/event-schedules/")
        ...
        and: "the scheduled event's event schedule id does not match the URL event schedule id"
            ScheduledEvent updatedEvent = CopyObjectUtil.scheduledEvent(initResponse.body())
            updatedEvent.setEventScheduleId("not-the-url-event schedule-id")
    ...
    def "Update a scheduled event: URL Event Schedule id exists: Update scheduled event is successful"() {
        given: "the event schedule id is in the url"
        ...
        and: "a scheduled event exists in the database"
            // Verify data seeded from Database init scripts correctly
            HttpResponse<ScheduledEvent> initResponse = httpClient.toBlocking()
                    .exchange(eventSchedulesUrl.concat(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getScheduledEventId()), ScheduledEvent)
            Assertions.assertEquals(HttpURLConnection.HTTP_OK, initResponse.code())
            Assertions.assertTrue(DateAndTimeComparisonUtil.areDateAndTimeEqual(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getCreatedOn(), initResponse.body().getCreatedOn()))
            Assertions.assertTrue(DateAndTimeComparisonUtil.areDateAndTimeEqual(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getLastUpdatedOn(), initResponse.body().getLastUpdatedOn()))

        and: "a valid update is made to scheduled event"
        ...
        when: "a request is made to with the updated scheduled event"
        ...
        and: "the updated scheduled event is returned in the response"
            ScheduledEvent returnedScheduledEvent = response.body()
            Assertions.assertEquals(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getEventScheduleId(), returnedScheduledEvent.getEventScheduleId())
            Assertions.assertEquals(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getScheduledEventId(), returnedScheduledEvent.getScheduledEventId())
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getLocationId(), returnedScheduledEvent.getLocationId())
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getOrganizerIds().get(0), returnedScheduledEvent.getOrganizerIds().get(0))
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getOrganizerIds().get(1), returnedScheduledEvent.getOrganizerIds().get(1))
            Assertions.assertEquals(ScheduledEventTestConstants.updateScheduledEventUpdatedName, returnedScheduledEvent.getName())
            Assertions.assertTrue(DateAndTimeComparisonUtil.areDateAndTimeEqual(TestConstants.updateCreatedOn, returnedScheduledEvent.getCreatedOn()))
            Assertions.assertTrue(DateAndTimeComparisonUtil.isDateAndTimeNow(returnedScheduledEvent.getLastUpdatedOn()))

        and: "the scheduled event is correct in the database"
            HttpResponse<ScheduledEvent> checkingDatabaseResponse = httpClient.toBlocking()
                    .exchange(eventSchedulesUrl.concat(returnedScheduledEvent.getScheduledEventId()), ScheduledEvent)

            Assertions.assertEquals(HttpURLConnection.HTTP_OK, checkingDatabaseResponse.code())

            ScheduledEvent databaseScheduledEvent = checkingDatabaseResponse.body()
            Assertions.assertEquals(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getEventScheduleId(), databaseScheduledEvent.getEventScheduleId())
            Assertions.assertEquals(ScheduledEventTestConstants.UPDATE_SCHEDULED_EVENT.getScheduledEventId(), databaseScheduledEvent.getScheduledEventId())
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getLocationId(), returnedScheduledEvent.getLocationId())
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getOrganizerIds().get(0), returnedScheduledEvent.getOrganizerIds().get(0))
            Assertions.assertEquals(ScheduledEventTestConstants.CREATE_EVENT.getOrganizerIds().get(1), returnedScheduledEvent.getOrganizerIds().get(1))
            Assertions.assertEquals(ScheduledEventTestConstants.updateScheduledEventUpdatedName, databaseScheduledEvent.getName())
            Assertions.assertTrue(DateAndTimeComparisonUtil.areDateAndTimeEqual(TestConstants.updateCreatedOn, databaseScheduledEvent.getCreatedOn()))
            Assertions.assertTrue(DateAndTimeComparisonUtil.isDateAndTimeNow(databaseScheduledEvent.getLastUpdatedOn()))
